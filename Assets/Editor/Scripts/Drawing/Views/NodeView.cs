...
namespace NodeEditor.Scripts.Views
{
	public class NodeView : Node
	{
		...
		VisualElement m_ErrorIndicator;
		...
		public void Initialize(AbstractNode inNode, IEdgeConnectorListener connectorListener)
		{
			...
			var errorIndicator = new VisualElement(){name = "error-indicator"};
			errorIndicator.visible = node.hasError;
			errorIndicator.RegisterCallback<MouseOverEvent>(e => GraphEditorView.tooltip = node.error);
			errorIndicator.RegisterCallback<MouseLeaveEvent>(e => GraphEditorView.tooltip = "");
			titleContainer.Insert(1, errorIndicator);
			m_ErrorIndicator = errorIndicator;
			...
			var controlsContainer = new VisualElement { name = "controls" };
			{
				...
				{
					var control = attribute.InstantiateControl(node, propertyInfo);
		...
		public void OnModified(ModificationScope scope)
		{
			...
			else if (scope == ModificationScope.ErrorValidation)
			{
				m_ErrorIndicator.visible = node.hasError;
			}
