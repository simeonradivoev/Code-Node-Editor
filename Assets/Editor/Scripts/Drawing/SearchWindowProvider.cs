...
using NodeEditor.Nodes.Expressions;
...
namespace NodeEditor.Scripts
{
    class SearchWindowProvider : ScriptableObject, ISearchWindowProvider
    {
        ...
        public void Initialize(EditorWindow editorWindow, AbstractNodeGraph graph, GraphView graphView)
        {
        ...
        public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
        {
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                {
                    if (type.IsClass && !type.IsGenericType && !type.IsAbstract && type.IsSubclassOf(typeof(AbstractNode)) 
                                                             && type != typeof(PropertyNode))
                    {
                        var attrs = type.GetCustomAttributes(typeof(TitleAttribute), true) as TitleAttribute[];
                        if (attrs != null && attrs.Length > 0)
                        {
                            var node = (AbstractNode)Activator.CreateInstance(type);
                            AddEntries(node, attrs.Reverse().SelectMany(a => a.title).ToArray(), nodeEntries);
                        }
                    }
                }
            }
