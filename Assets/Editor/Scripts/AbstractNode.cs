...
namespace NodeEditor
{
	public class AbstractNode : INode, ISerializationCallbackReceiver, IDisposable
	{
		...
		public void GetSlotValues<T>(ISlot inputSlot,IList<T> values)
		{
			...
			{
				var hasValue = inputSlot as IHasValue<T>;
				if(hasValue != null) values.Add(hasValue.value);
			}

			ListPool<IEdge>.Release(edges);
		}

		public IList<T> GetSlotValues<T>(ISlot inputSlot)
		{
			List<T> values = new List<T>();

			GetSlotValues<T>(inputSlot, values);
		...
		public virtual void ValidateNode()
		{
			...
			s_TempSlots.Clear();
			GetOutputSlots(s_TempSlots);
			isInError |= CalculateNodeHasError(ref error);
			Dirty(ModificationScope.ErrorValidation);
			hasError = isInError;
			m_Error = error;
		...
		//True if error
		protected virtual bool CalculateNodeHasError(ref string error)
		{
			return false;
		}
